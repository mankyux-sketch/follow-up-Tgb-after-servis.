...
const fileInput=document.getElementById('excelFile');fileInput.onchange=async()=>{selectedFile=fileInput.files[0];if(!selectedFile)return;const buf=await selectedFile.arrayBuffer();selectedWorkbook=XLSX.read(buf,{type:'array',cellDates:true});const ws=selectedWorkbook.Sheets[selectedWorkbook.SheetNames[0]],rows=XLSX.utils.sheet_to_json(ws,{defval:''});document.getElementById('fileInfo').textContent=`${selectedFile.name} • Sheet: ${selectedWorkbook.SheetNames[0]} • ${rows.length} baris`;document.getElementById('importBtn').disabled=false}
...
document.getElementById('cancelTemplateBtn').onclick=clearTemplateForm;function clearTemplateForm(){document.getElementById('templateName').value='';document.getElementById('templateText').value='';document.getElementById('editingTemplateId').value=''}
